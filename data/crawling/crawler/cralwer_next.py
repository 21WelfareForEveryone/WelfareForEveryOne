...
make_csv=pd.DataFrame(columns=['title', 'summary', 'who', 'criteria', 'what', 'how', 'calls', 'sites','category'])
...
for div in range(3,7):
        ...
        for page in tqdm(range(1,page_num+1),mininterval=1):
            ...
            try:
                ...
                for block in tqdm(range(1,11),mininterval=1):
                    ...
                    make_csv = make_csv.append(dfNew, ignore_index=True)
    ...
    make_csv=make_csv.drop_duplicates(['title'],keep='first')
    make_csv.to_csv("./data/"+category_name+".csv", index=False, encoding='utf-8-sig')
    make_csv=pd.DataFrame(columns=['title', 'summary', 'who', 'criteria', 'what', 'how', 'calls', 'sites','category'])
